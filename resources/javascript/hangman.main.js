    ...
    var $inputWord, $inputLetter, $wordBankDiv, $strikes, $finalWord, $guessWord
        ...
        obj = {
            ...
            init: function(){
				...
				$guessWord.submit(function(e){
					e.preventDefault();
					var word = this['guessWordField'].value;
					$.ajax({
						url: settings.serverUrl + '/guessWord',
						context: Hangman,
						data: {
							player: Hangman.player,
							word: word
            ...
            loseGame: function(){
				...
				if(this.player.isMaster){
            		...
            		new Hangman.Alert("Sorry, you lose; the correct word was \"" + $finalWord + "\"");
            	}            
            ...
            loseGameGuessingWord: function(loser){
            	if(this.player.name === players[loser].name){
            		new Hangman.Alert("Sorry, you're disqualified for this round.");
            		this.player.isActive = false;
            	}
            	
            	if(players[loser].name == this.player.name){
					var j, count = 0;
					for(j in players){
						if(players[j].isActive && !players[j].isMaster && (players[j].name != players[loser].name)){
							$('#' + j + 'ChooseBtn').show();
							count = count + 1;
						}else{
							$('#' + j + 'ChooseBtn').hide();
					...
					if(count >= 2){
						...
						var j;
						for(j in players){
							if(players[j].isActive && !players[j].isMaster){
								$.ajax({
									url: settings.serverUrl + '/giveTurn',
									context: Hangman,
									data: {
										player: Hangman.player,
										choice: players[j].name
									},
									success: function(data, textStatus, jqXHR){
										return;
									}
            ...
            winGameGuessingWord: function(winner){
            	players[winner].score = players[winner].score + 1;
            	
            	if(this.player.name === players[winner].name){
            		new Hangman.Alert("You win!");
            	}else{
					new Hangman.Alert("Sorry, " + players[winner].name + " wins. The correct word was \"" + $finalWord + "\"");
            	}
            ...
            winGame: function(){
            	...
            	if(this.player.isGuessing){
					...
					for(k in players){
						if(players[k].isGuessing){
							new Hangman.Alert("Sorry, " + players[k].name + " wins. The correct word was \"" + $finalWord + "\"");
						}
					}
            	}
                        ...
                        success: function(data, textStatus, jqXHR){
                            ...
                            for (; i < e; i++){
                                if (data[i].action === 'connected'){
                                	...
                                	//new Hangman.Alert(data[i].data.player.name + ' guessed the word: ' + data[i].data.word + correct);
                                	var k;
                                	for(k in players){
                                		if(players[k].name === data[i].data.player.name){
											if(data[i].data.correct){
												Hangman.winGameGuessingWord(k);
											}else{
												Hangman.loseGameGuessingWord(k);
											}
										}	
									}
